...
class UISnapshot:
    ...
    sel_bld_info: Optional[dict] = None
...
class UIManager:
    ...
    def draw_all(self, screen: pygame.Surface, snap: UISnapshot) -> None:
        ...
        if getattr(snap, "selected_slot", None) is not None and getattr(snap, "sel_bld_info", None):
            try:
                self._draw_selection_panel(screen, snap.sel_bld_info)
            except Exception as _e:
                print(f"[UIManager] _draw_selection_panel error: {_e}")
        try:
    ...
    def draw_unit_info(self, screen: pygame.Surface) -> None:
        ...
        # ── Swarm unit data ───────────────────────────────────────────────────
        _SWARM_UNITS = [
            {"kind": "crawler", "label": "Crawler", "zh": "爬行者", "accent": (140, 220, 80), "icon": "▼", "stats": [("生命 HP", "30"), ("移速 Speed", "3.5 (極快)"), ("傷害 ATK", "15"), ("攻速 CD", "0.6s"), ("射程 Scan", "40 (近戰)"), ("產量 Spawn", "x2 雙胞胎")], "atk": "普通 Normal", "armor": "輕甲 Light", "desc": "極致的近戰炮灰，每次孵化兩隻，以數量壓制敵人。", "built": "酸液繁殖池 Acid Pool"},
            {"kind": "spitter", "label": "Spitter", "zh": "吐酸者", "accent": (180, 255, 50), "icon": "◉", "stats": [("生命 HP", "40"), ("移速 Speed", "2.5"), ("傷害 ATK", "20 腐蝕"), ("攻速 CD", "1.2s"), ("射程 Scan", "200"), ("產量 Spawn", "x2 雙胞胎 / 可對空")], "atk": "腐蝕 Acid", "armor": "輕甲 Light", "desc": "基礎遠程防空蟲，成群結隊形成酸液火網。", "built": "毒素腔室 Toxin Chamber"},
